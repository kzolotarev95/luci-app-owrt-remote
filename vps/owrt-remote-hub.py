...
.brandPanel{{display:grid;grid-template-columns:repeat(2,minmax(0,132px));gap:8px}}.brandPanel .appBanner{{grid-column:1/-1;width:100%;min-width:0}}.brandPanel .links{{grid-column:1/-1;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:0}}.brandPanel .links a{{width:100%;min-width:0}}.card{{text-align:center}}.card.hasStats{{padding-top:58px}}.cardStats{{position:absolute;left:12px;top:12px;z-index:2;display:flex;gap:7px;flex-wrap:wrap;max-width:calc(100% - 24px)}}.cardStats .miniStat{{min-width:0;min-height:30px;padding:7px 10px;font-size:12px;background:rgba(255,255,255,.08)}}.cardTop{{align-items:center;justify-content:center;flex-direction:column}}.routerMark{{margin:0 auto}}.tagRow,.actions{{justify-content:center}}.name{{white-space:normal}}.metric{{text-align:center}}.metric.span2{{grid-column:1/-1}}
...
function summaryStats(list) {{
  ...
  return {{
    total: total + ' ' + plural(total, 'роутер', 'роутера', 'роутеров'),
    online: online + ' онлайн',
    offline: (total - online) + ' оффлайн'
  }};
}}

function statsHtml(stats) {{
  return `<div class="cardStats">
    <span class="miniStat">${{escapeHtml(stats.total)}}</span>
    <span class="miniStat">${{escapeHtml(stats.online)}}</span>
    <span class="miniStat">${{escapeHtml(stats.offline)}}</span>
  </div>`;
}}
...
function render(list) {{
  ...
  const mainRouter = list.find(r => String(r.role || 'node') === 'main') || list[0];
  cards.innerHTML = list.map(r => {{
    const role = String(r.role || 'node');
    const isMain = role === 'main';
    const showStats = mainRouter && r.id === mainRouter.id;
    ...
    return `<article class="card ${{isMain ? 'main' : ''}} ${{showStats ? 'hasStats' : ''}} ${{online ? 'online' : 'off'}}">
      ${{showStats ? statsHtml(stats) : ''}}
      <div class="cardTop">
